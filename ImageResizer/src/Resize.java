...
public class Resize extends Thread {
    ...
    private long start;
    private int newHight = 500;

    public Resize(File[] files, int newWidth, String dstFolder, long start) {
        this.files = files;
    ...
    public void run() {
        try {
            for (File file : files) {
                ...
                BufferedImage newImage = resize(image, newWidth, newHight, Scalr.OP_ANTIALIAS);
